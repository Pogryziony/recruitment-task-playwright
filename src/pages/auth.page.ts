...
export class AuthPage {
  ...
  async register(user: { username: string; email: string; password: string }, opts?: { skipPrivacy?: boolean }) {
    ...
    // Some runs show the select value being reset to the placeholder; do a small retry loop.
    for (let attempt = 0; attempt < 3; attempt++) {
      await this.region().selectOption({ index: 1 }).catch(() => {
        // ignore and try JS fallback below
      });

      let selectedText = (await this.region().locator('option:checked').textContent()) ?? '';
      if (/please select/i.test(selectedText)) {
        // Fallback: force selection via DOM in case the site's scripts interfere with Playwright's selectOption.
        ...
        selectedText = (await this.region().locator('option:checked').textContent()) ?? '';
      }
      if (!/please select/i.test(selectedText)) {
        break;
      }
